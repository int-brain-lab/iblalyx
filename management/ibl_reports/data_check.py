...
def behaviour_data_status(datasets, probe):

    data = get_data_status(datasets, expected_data.BEHAVIOUR, 'Raw behaviour data')
    data['tasks'] = get_tasks(expected_data.BEHAVIOUR_TASKS, probe)
...
def get_data_status_qs(probe_insertions):
    ...
    critical_passive = [dset[0] for dset in expected_data.PASSIVE if dset[2]]
    critical_video = [dset[0] for dset in expected_data.VIDEO if dset[2]]
