...
logger = logging.getLogger('ibllib')  # FIXME Should use Django logger
ROOT = Path('/mnt/ibl')
...
def generate_trials_aggregate(subject, sessions, root):
    """
    ...
    -------
    pandas.DataFrame
        A sorted dataframe with the columns TODO.
    pandas.DataFrame
        A table of extraction outcomes TODO.
    set of pathlib.Path
        A list of input files used by the extractor tasks.
    """
    outcomes = []
    all_trials = []
    input_files = set()
    ...
    for i, info in sessions.iterrows():
        ...
        for task in tasks:
            ...
            try:
                j = 0 if task.collection == 'raw_behavior_data' else int(task.collection.split('_')[-1])
            except Exception:
...
def generate_aggregate(session_tasks: dict, outcomes=None):
    """
    Extract trials tables for a given set of sessions.
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('output-path', type=Path, help='The save location of the aggregate dataset.')
        ...
        parser.add_argument('--clobber', store_true=True, default=False,
    ...
    def handle(self, *_, **options):
        ...
        # TODO remove old way
        # all_trials, outcomes, input_files = generate_trials_aggregate(subject, sessions, options['data_path'])
        ...
        if not rerun:
            ...
            if not rerun:
                ...
                rerun |= sessions['íd'].astype(str).unique() != old_trials['session'].unique()
        ...
        if not rerun:
            logger.info('No reason to rerun')  # TODO more detail
            return

        # Generate aggregate trials table
        all_trials, outcomes = generate_aggregate(all_tasks, outcomes)
        outcomes = pd.DataFrame(outcomes, columns=['session', 'task number', 'notes'])
        # Add start_times column to trials table
        start_times = sessions.set_index('id')['start_time'].rename('session_start_time')
        ...
        # Create dataset
        self.register_dataset(out_file, file_hash=hash, file_size=file_size, aggregate_hash=aggregate_hash, user=user)
    ...
    def files2datasets(file_list):
        ...
        for session_path, datasets in files_by_session.items():
            ...
            if sessions.count():
                ...
                dsets = session_datasets.values('hash', 'name', 'collection', 'revision__name')
                dsets = [d for d in dsets
                         if ('/'.join(filter(None, [d['collection'], d['revision__name']])) + '/' + d['name']) in datasets]
    ...
    def make_aggregate_hash(sessions, input_files=None):
        ...
        if input_files:  # TODO get hashes from db instead
            input_files = sorted(set(input_files))
            assert all(map(Path.exists, input_files))
            trials_hashes = filter(None, trials_ds.order_by('hash').values_list('hash', flat=True))
            inputs_hashes = map(hashfile.md5, input_files)
            ...
            # If we don't have task data for each session, that's a problem. TODO This is a little unnecessary
    ...
    def register_dataset(self, file_path, file_hash=None, aggregate_hash=None, file_size=None, user='root', revision=None):
        ...
        dset, is_new = Dataset.objects.get_or_create(
            ...
            data_format=DataFormat.objects.get(name='parquet'), content_object=self.subject)
        ...
        logger.info(('Created' if is_new else 'Updated') + ' aggregate dataset with UUID', dset.pk)
        # Validate dataset
        dset.full_clean()
        dset.save()
