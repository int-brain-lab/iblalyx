...
# Here we are only excluding one dataset type, in the future dataset types should be specified explicitly
dtypes_exclude = ['_iblrig_taskSettings.raw']
probe_insertions = ProbeInsertion.objects.filter(id__in=pids)
datasets = Dataset.objects.filter(
    session__in=probe_insertions.values_list('session', flat=True)).exclude(dataset_type__name__in=dtypes_exclude)
