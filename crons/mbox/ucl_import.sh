...
docker exec -i cortexlab_buffer_postgres psql -q -U ibl_dev -d postgres -c "CREATE DATABASE cortexlab;" 2>/dev/null || true
# Drop and recreate schema
docker exec -i cortexlab_buffer_postgres psql -q -U ibl_dev -d cortexlab -c "drop schema public cascade; create schema public"
...
| docker exec -i cortexlab_buffer_postgres psql -U ibl_dev -d cortexlab
